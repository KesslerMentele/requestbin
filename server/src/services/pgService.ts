...
const binExists = async (binName: string): Promise<boolean> => {
    const result = await postgresPool.query<{ exists: boolean }>(
      `SELECT EXISTS (
          SELECT 1 FROM bins WHERE name = $1
      ) AS exists`,
      [binName]
    );
...
const getAllRequests = async (binName: string): Promise<RequestRow[]> => {
    ...
    `select requests.id, bin.name as bin_name,
       requests.mongodb_id, requests.time_stamp, requests.http_method FROM "requests" 
       JOIN bins bin on requests.bin_id = bin.id WHERE bin.name = $1`,
      [binName]
    );
    return result.rows;
}
...
const pgService: PostgresService = {
    getAllBins,
    getAllRequests,
    createBin,
    deleteBin,
